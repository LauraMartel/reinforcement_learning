...
class dungeon_map:
    ...
    def __init__(self, size, walls, learning_rate, discount,
                 epsilon, reward_treasure, punition_walls, reward_exit):
        ...
        # weight measure of how important do we find future actions
        # vs current action or future reward over current reward. values between 0 and 1
        self.gamma = discount
        ...
        # and force the agent to go to the treasure (not stay blocked) then to the exit.
    ...
    def layout(self):
        ...
        # treasure, I put 0 for the treasure for the ball to pass by the value, but not stay there.
        ...
        while self.penalities != 0:
            ...
            if self.reward[i,j] == -1 and [i,j] != [0,0] and [i,j] != [self.n-1,self.n-1] and [i,j]!=[1,2]:
                ...
                # keep at -1 for the 4 walls. You can also modify it to change the number of walls.
                self.penalities -= 1
                # colors of the cases with penalties
                self.colors[self.n*i+j] = (255,0,0)  #red
                # if we place terminals here, the agent will not explore all the maze and exclude
                # some parts. I feel that somehow it is slower like that.
                # As I want it to explore everything, I put the terminals in the function episode under.
                # but you can change.
    ...
    # Function to make the agent move.
    def select_action(self, current_state):
        # create an empty list to store the future actions
        possible_actions = []

        # if a random number is <= to epsilon you can move and predict next action
        ...
        # if not, you use the Q table to predict next action
        else:
    ...
    def episode(self):
        ...
        # create the initial current state
...
# the size of the cell (here 4x4 but can be any square matrix),
# the number of walls, the learning rate, the discount, the epsilon
# the reward/penalty for the treasure, the walls penalty, the exit reward
# you can change these values as you like.
dm = dungeon_map(4, 4, 0.2, 0.9, 0.1, 0, -10, 10)


# make the program run in an infinite loop
# I noticed that the 1st iteration/epoch is often not good with the start or the exit being blocked by the walls.
# Restart a new one in this case.
run = True
while run:
    sleep(0.1)
